...
impl PacketSender {
    ...
    pub async fn send_packets_with_timing(interface: &NetworkInterface, packets: Vec<(DateTime<Utc>, Vec<u8>)>) -> Result<(), PacketReaderError> {
        if packets.is_empty() {
            info!("送信するパケットがありません");
            return Ok(());
        }
